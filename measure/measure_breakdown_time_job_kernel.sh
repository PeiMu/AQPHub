...
engine=$1
split=$2
kernel_path=${3:-pipeline}  # none | pipeline | query
jit_opt=${4:-o1}
jit_simd=${5:-auto}
fusion_probe=${6:-on}
inline_hash=${7:-on}
...
mv "${log_name}" job_result/${engine}_${split}_kernel-${kernel_path}_${jit_opt}_${jit_simd}${flag_suffix}_breakdown_"${log_name}"
