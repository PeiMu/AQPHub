...
# ============================================================
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off off off llvm && \
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off off off fastisel && \
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off off off tpde && \
...
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off single-run-strict off llvm && \
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off single-run-strict off fastisel && \
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off single-run-strict off tpde && \

# ---- cache=single-run-template, spec=off (3) ----
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off single-run-template off llvm && \
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off single-run-template off fastisel && \
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off single-run-template off tpde && \
...
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off full off llvm && \
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off full off fastisel && \
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off full off tpde && \

# ---- cache=off, spec=recompile (3) ----
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off off recompile llvm && \
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off off recompile fastisel && \
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off off recompile tpde && \

# ---- cache=single-run-strict, spec=recompile (3) ----
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off single-run-strict recompile llvm && \
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off single-run-strict recompile fastisel && \
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off single-run-strict recompile tpde && \

# ---- cache=single-run-template, spec=recompile (3) ----
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off single-run-template recompile llvm && \
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off single-run-template recompile fastisel && \
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off single-run-template recompile tpde && \
...
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off full recompile llvm && \
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off full recompile fastisel && \
bash ./measure_breakdown_time_job.sh duckdb node-based query none on on on off full recompile tpde && \
