...
def main():
    ...
    CONFIGS = [
        ...
        ("expr (nospec)",
         f"duckdb_{split}_expr_none_nospecjit_breakdown_time_log.csv", True),
        ("expr_simd (nospec)",
         f"duckdb_{split}_expr_auto_nospecjit_breakdown_time_log.csv", True),
        ("operator",
         f"duckdb_{split}_operator_none_breakdown_time_log.csv", True),
        ("operator_simd",
         f"duckdb_{split}_operator_auto_breakdown_time_log.csv", True),
        ("operator (nospec)",
         f"duckdb_{split}_operator_none_nospecjit_breakdown_time_log.csv", True),
        ("operator_simd (nospec)",
         f"duckdb_{split}_operator_auto_nospecjit_breakdown_time_log.csv", True),
        ("pipeline",
         f"duckdb_{split}_pipeline_none_breakdown_time_log.csv", True),
        ("pipeline_simd",
         f"duckdb_{split}_pipeline_auto_breakdown_time_log.csv", True),
        ("pipeline (nospec)",
         f"duckdb_{split}_pipeline_none_nospecjit_breakdown_time_log.csv", True),
        ("pipeline_simd (nospec)",
         f"duckdb_{split}_pipeline_auto_nospecjit_breakdown_time_log.csv", True),
        ("query_full",
