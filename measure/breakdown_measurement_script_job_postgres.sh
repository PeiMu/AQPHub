#!/usr/bin/env bash
#
# PostgreSQL full performance sweep: 73 configs.
...
#   + 1 topdown   × (3 compile-mode + 1 tune) × 4 cache
#                  × 2 spec(off/recompile)                 = 32
#   = 73 configs
#
...
# 1 interpreter + (3 compile-mode + 1 tune) × 4 cache × 2 spec = 33
# ============================================================

# ---- interpreter baseline ----
run_pg_breakdown topdown none && \
...
# ---- cache=off, spec=off: 3 compile-mode + 1 tune = 4 ----
run_pg_breakdown topdown query none all off off llvm && \
run_pg_breakdown topdown query none all off off fastisel && \
run_pg_breakdown topdown query none all off off tpde && \

# Generate tune JSON from cache=off CSVs produced above
python3 tune_per_subquery.py topdown --engine=postgresql && \

run_pg_breakdown topdown query none all off off llvm job_result/tuned_per_subquery_topdown.json && \

# ---- cache=single-run-strict, spec=off: 4 ----
run_pg_breakdown topdown query none all single-run-strict off llvm && \
run_pg_breakdown topdown query none all single-run-strict off fastisel && \
run_pg_breakdown topdown query none all single-run-strict off tpde && \
run_pg_breakdown topdown query none all single-run-strict off llvm job_result/tuned_per_subquery_topdown.json && \

# ---- cache=single-run-template, spec=off: 4 ----
run_pg_breakdown topdown query none all single-run-template off llvm && \
run_pg_breakdown topdown query none all single-run-template off fastisel && \
run_pg_breakdown topdown query none all single-run-template off tpde && \
run_pg_breakdown topdown query none all single-run-template off llvm job_result/tuned_per_subquery_topdown.json && \

# ---- cache=full, spec=off: 4 ----
run_pg_breakdown topdown query none all full off llvm && \
run_pg_breakdown topdown query none all full off fastisel && \
run_pg_breakdown topdown query none all full off tpde && \
run_pg_breakdown topdown query none all full off llvm job_result/tuned_per_subquery_topdown.json && \
...
# ---- cache=off, spec=recompile: 4 ----
run_pg_breakdown topdown query none all off recompile llvm && \
run_pg_breakdown topdown query none all off recompile fastisel && \
run_pg_breakdown topdown query none all off recompile tpde && \
run_pg_breakdown topdown query none all off recompile llvm job_result/tuned_per_subquery_topdown.json && \

# ---- cache=single-run-strict, spec=recompile: 4 ----
run_pg_breakdown topdown query none all single-run-strict recompile llvm && \
run_pg_breakdown topdown query none all single-run-strict recompile fastisel && \
run_pg_breakdown topdown query none all single-run-strict recompile tpde && \
run_pg_breakdown topdown query none all single-run-strict recompile llvm job_result/tuned_per_subquery_topdown.json && \

# ---- cache=single-run-template, spec=recompile: 4 ----
run_pg_breakdown topdown query none all single-run-template recompile llvm && \
run_pg_breakdown topdown query none all single-run-template recompile fastisel && \
run_pg_breakdown topdown query none all single-run-template recompile tpde && \
run_pg_breakdown topdown query none all single-run-template recompile llvm job_result/tuned_per_subquery_topdown.json && \

# ---- cache=full, spec=recompile: 4 ----
run_pg_breakdown topdown query none all full recompile llvm && \
run_pg_breakdown topdown query none all full recompile fastisel && \
run_pg_breakdown topdown query none all full recompile tpde && \
run_pg_breakdown topdown query none all full recompile llvm job_result/tuned_per_subquery_topdown.json && \

echo "=== All 73 PostgreSQL breakdown measurements complete ==="
