...
if [[ "$generate_golden" != "--test-only" ]]; then
  ...
  echo "========================================"
  bash run_aqp.sh "dsb_${DSB_SF}" postgresql none none none
  cp "${RESULT_DIR}/aqp_middleware_postgresql_none_none_none_dsb.txt" "${GOLDEN_NOSPLIT}"
  echo ""
...
for entry in "${JIT_CONFIGS[@]}"; do
  ...
  if [[ "$jit_cache_mode" == "full" ]]; then
    golden_filtered=$(filter_output "$golden")
    golden_lines=$(echo "$golden_filtered" | wc -l)
    d0=$(diff <(filter_output "$output" | head -n "$golden_lines") <(echo "$golden_filtered") || true)
    if [[ -z "$d0" ]]; then
      echo "  PASS (iter0)"
      ((passed++))
    else
...
if [[ -f "$TUNE_JSON" ]]; then
  ...
  output="${RESULT_DIR}/aqp_middleware_postgresql_auto_query_none_jitcache_single_run_parameterized_tuned_dsb.txt"
  if [[ ! -f "$output" ]]; then
    echo "  FAIL: output file not found: $output"
    FAILED_CONFIGS+=("$config_label  [output missing: $output]")
    echo "--- $config_label ---" >> "$FAIL_LOG"
...
#       on on on on full off llvm "$TUNE_JSON"
  ...
  output="${RESULT_DIR}/aqp_middleware_postgresql_auto_query_none_jitcache_single_run_parameterized_specrecompile_tuned_dsb.txt"
  if [[ ! -f "$output" ]]; then
    echo "  FAIL: output file not found: $output"
    FAILED_CONFIGS+=("$config_label  [output missing: $output]")
    echo "--- $config_label ---" >> "$FAIL_LOG"
