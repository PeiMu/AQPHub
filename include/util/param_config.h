...
struct ParamConfig {
  ...
  // Runtime-statistics-guided optimizations (node-based split).
  // Each defaults ON; disable with --no-<name>.
  bool range_predicate_injection = true;
  bool bloom_filter_injection = true;
  ...
  // optimizer is skipped. Set via --disable-optimizer.
  bool disable_engine_optimizer = false;
