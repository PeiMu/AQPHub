    ...
    (void)jd.define(absoluteSymbols({
        ...
        {es.intern("memset"),
         JITEvaluatedSymbol(pointerToJITTargetAddress((void *)&memset),
                            JITSymbolFlags::Exported)},
        // Pipeline kernel builder helpers — registered lazily in
        // CompilePipelineKernel (addresses come from kernel_lib, not this TU)
    }));
  }
...
// ---------------------------------------------------------------------------
// Level 4: Pipeline kernel probe loop JIT compilation.
// Generates one function per PipelineKernelPlan that fuses scan filters,
// hash probe, and output emission into a single native function.
// ---------------------------------------------------------------------------

AQPPipelineKernelFn
IrToLlvmCompiler::CompilePipelineKernel(
    const middleware::storage::PipelineKernelPlan &plan) {

  using middleware::storage::FlatColumnType;
  using middleware::storage::PipelineFilterDesc;
  using middleware::storage::KernelOutputCol;

  if (!plan.valid) return nullptr;

  // Lazily register pipeline kernel builder helpers (addresses resolved via
  // dlsym to avoid a link-time dependency from aqp_jit_lib to kernel_lib).
  {
    static bool pk_syms_registered = false;
    if (!pk_syms_registered) {
      void *h_int = dlsym(RTLD_DEFAULT, "aqp_pk_builder_append_int");
      void *h_str = dlsym(RTLD_DEFAULT, "aqp_pk_builder_append_str");
      void *h_row = dlsym(RTLD_DEFAULT, "aqp_pk_builder_finish_row");
      if (h_int && h_str && h_row) {
        auto &es = impl_->jit->getExecutionSession();
        auto &jd = impl_->jit->getMainJITDylib();
        cantFail(jd.define(absoluteSymbols({
          {es.intern("aqp_pk_builder_append_int"),
           JITEvaluatedSymbol(pointerToJITTargetAddress(h_int),
                              JITSymbolFlags::Exported)},
          {es.intern("aqp_pk_builder_append_str"),
           JITEvaluatedSymbol(pointerToJITTargetAddress(h_str),
                              JITSymbolFlags::Exported)},
          {es.intern("aqp_pk_builder_finish_row"),
           JITEvaluatedSymbol(pointerToJITTargetAddress(h_row),
                              JITSymbolFlags::Exported)},
        })));
        pk_syms_registered = true;
      }
    }
  }

  uint64_t fn_id = s_filter_counter.fetch_add(1, std::memory_order_relaxed);
  std::string fn_name = "aqp_pk_probe_" + std::to_string(fn_id);

  auto ctx = std::make_unique<LLVMContext>();
  auto mod = std::make_unique<Module>("aqp_pk_mod", *ctx);
  ...
  // AQPPipelineKernelView struct type:
  // { scan_nrows:i64, scan_col_data:i8**, scan_str_pools:i8**,
  //   scan_null_bitmaps:i64**, num_join_steps:i32,
  //   ht_buckets:i32**, ht_next:i32**, ht_keys:i32**, ht_row_ids:i32**,
  //   ht_masks:i32*, ht_sizes:i32*,
  //   build_col_data:i8***, build_str_pools:i8***,
  //   bf_data:i64**, bf_masks:i64* }
  // We access it via byte offsets with GEP on i8*, avoiding struct definition.

  // Function: uint64_t fn(uint64_t start, uint64_t end, const void *view, void *builder)
  FunctionType *fn_ty = FunctionType::get(
      i64, {i64, i64, i8p, i8p}, false);
  Function *fn = Function::Create(
      fn_ty, Function::ExternalLinkage, fn_name, mod.get());
  fn->setDoesNotThrow();

  Value *start_arg = fn->getArg(0); start_arg->setName("start");
  Value *end_arg   = fn->getArg(1); end_arg->setName("end");
  Value *view_arg  = fn->getArg(2); view_arg->setName("view");
  Value *builder_arg = fn->getArg(3); builder_arg->setName("builder");

  BasicBlock *entry_bb = BasicBlock::Create(*ctx, "entry", fn);
  BasicBlock *loop_bb = BasicBlock::Create(*ctx, "loop", fn);
  BasicBlock *loop_footer = BasicBlock::Create(*ctx, "loop_footer", fn);
  BasicBlock *exit_bb = BasicBlock::Create(*ctx, "exit", fn);

  IRBuilder<> b(entry_bb);

  // === Entry: Load view pointers ===
  // We model AQPPipelineKernelView as a flat struct.
  // For simplicity, load fields by casting view_arg to the right pointer type
  // at known byte offsets. On x86-64: pointers=8 bytes, uint32_t=4 bytes.
  //
  // Offset layout (x86-64):
  //  0: scan_nrows (i64)
  //  8: scan_col_data (ptr)
  // 16: scan_str_pools (ptr)
  // 24: scan_null_bitmaps (ptr)
  // 32: num_join_steps (i32)  [padded to 8]
  // 40: ht_buckets (ptr)
  // 48: ht_next (ptr)
  // 56: ht_keys (ptr)
  // 64: ht_row_ids (ptr)
  // 72: ht_masks (ptr)
  // 80: ht_sizes (ptr)
  // 88: build_col_data (ptr)
  // 96: build_str_pools (ptr)
  // 104: bf_data (ptr)
  // 112: bf_masks (ptr)

  auto loadViewField = [&](unsigned byte_offset, Type *pointee_ty,
                           const Twine &name) -> Value * {
    Value *field_ptr = b.CreateBitCast(
        b.CreateGEP(i8, view_arg, ConstantInt::get(i64, byte_offset)),
        PointerType::getUnqual(pointee_ty));
    return b.CreateLoad(pointee_ty, field_ptr, name);
  };

  Type *i8pp = PointerType::getUnqual(i8p);
  Type *i8ppp = PointerType::getUnqual(i8pp);
  Type *i32pp = PointerType::getUnqual(i32p);

  // Scan column data pointers
  Value *scan_col_data_base = loadViewField(8, i8pp, "scan_col_data");
  Value *scan_str_pools_base = loadViewField(16, i8pp, "scan_str_pools");
  Value *scan_null_bitmaps_base = loadViewField(24, PointerType::getUnqual(i64p), "scan_null_bm");

  // HT arrays
  Value *ht_buckets_base = loadViewField(40, i32pp, "ht_buckets_base");
  Value *ht_next_base = loadViewField(48, i32pp, "ht_next_base");
  Value *ht_keys_base = loadViewField(56, PointerType::getUnqual(PointerType::getUnqual(i32)), "ht_keys_base");
  Value *ht_row_ids_base = loadViewField(64, i32pp, "ht_row_ids_base");
  Value *ht_masks_base = loadViewField(72, i32p, "ht_masks_base");

  // Build col data
  Value *build_col_data_base = loadViewField(88, i8ppp, "build_col_data_base");
  Value *build_str_pools_base = loadViewField(96, i8ppp, "build_str_pools_base");

  // Hoist per-step HT pointers into entry block SSA values
  ...
  std::vector<StepSSA> step_ssa(nsteps);

  for (size_t si = 0; si < nsteps; si++) {
    Constant *si_c = ConstantInt::get(i64, si);
    step_ssa[si].buckets = b.CreateLoad(
        i32p, b.CreateGEP(i32p, ht_buckets_base, si_c),
        "ht_buckets_" + std::to_string(si));
    step_ssa[si].next = b.CreateLoad(
        i32p, b.CreateGEP(i32p, ht_next_base, si_c),
        "ht_next_" + std::to_string(si));
    step_ssa[si].keys = b.CreateLoad(
        i32p, b.CreateGEP(i32p, ht_keys_base, si_c),
        "ht_keys_" + std::to_string(si));
    step_ssa[si].row_ids = b.CreateLoad(
        i32p, b.CreateGEP(i32p, ht_row_ids_base, si_c),
        "ht_row_ids_" + std::to_string(si));
    step_ssa[si].mask = b.CreateLoad(
        i32, b.CreateGEP(i32, ht_masks_base, si_c),
        "ht_mask_" + std::to_string(si));
  }

  // Hoist scan column data pointers for columns used by filters, probes, and output
  std::unordered_map<int, Value *> scan_col_ptrs; // col_idx → i8*
  auto ensureScanCol = [&](int col_idx) {
    if (scan_col_ptrs.count(col_idx)) return;
    scan_col_ptrs[col_idx] = b.CreateLoad(
        i8p, b.CreateGEP(i8p, scan_col_data_base, ConstantInt::get(i64, col_idx)),
        "scan_col_" + std::to_string(col_idx));
  };

  // Hoist scan str pool pointers for VARCHAR columns
  std::unordered_map<int, Value *> scan_pool_ptrs;
  auto ensureScanPool = [&](int col_idx) {
    if (scan_pool_ptrs.count(col_idx)) return;
    scan_pool_ptrs[col_idx] = b.CreateLoad(
        i8p, b.CreateGEP(i8p, scan_str_pools_base, ConstantInt::get(i64, col_idx)),
        "scan_pool_" + std::to_string(col_idx));
  };

  // Hoist build col data pointers for inner join output
  // build_col_ptrs[step][col_idx] = i8*
  std::vector<std::unordered_map<int, Value *>> build_col_ptrs(nsteps);
  auto ensureBuildCol = [&](size_t step, int col_idx) {
    if (build_col_ptrs[step].count(col_idx)) return;
    Value *step_cols = b.CreateLoad(
        i8pp, b.CreateGEP(i8pp, build_col_data_base, ConstantInt::get(i64, step)),
        "build_cols_" + std::to_string(step));
    build_col_ptrs[step][col_idx] = b.CreateLoad(
        i8p, b.CreateGEP(i8p, step_cols, ConstantInt::get(i64, col_idx)),
        "build_col_" + std::to_string(step) + "_" + std::to_string(col_idx));
  };

  std::vector<std::unordered_map<int, Value *>> build_pool_ptrs(nsteps);
  auto ensureBuildPool = [&](size_t step, int col_idx) {
    if (build_pool_ptrs[step].count(col_idx)) return;
    Value *step_pools = b.CreateLoad(
        i8pp, b.CreateGEP(i8pp, build_str_pools_base, ConstantInt::get(i64, step)),
        "build_pools_" + std::to_string(step));
    build_pool_ptrs[step][col_idx] = b.CreateLoad(
        i8p, b.CreateGEP(i8p, step_pools, ConstantInt::get(i64, col_idx)),
        "build_pool_" + std::to_string(step) + "_" + std::to_string(col_idx));
  };

  // Pre-hoist all needed column pointers (recursive for LOGICAL_AND/OR/NOT)
  std::function<void(const PipelineFilterDesc &)> hoistFilterCols =
      [&](const PipelineFilterDesc &fd) {
    if (fd.col_idx >= 0) ensureScanCol(fd.col_idx);
    if (fd.col_idx >= 0 && fd.col_type == FlatColumnType::VARCHAR) ensureScanPool(fd.col_idx);
    for (const auto &child : fd.children) hoistFilterCols(child);
  };
  for (const auto &fd : plan.scan_filter_descs) hoistFilterCols(fd);
  for (size_t si = 0; si < nsteps; si++) {
    const auto &step = plan.join_steps[si];
    if (step.scan_key_col >= 0) ensureScanCol(step.scan_key_col);
  }
  for (const auto &out : plan.output_cols) {
    if (out.source == KernelOutputCol::FROM_SCAN) {
      ensureScanCol(out.col_idx);
      if (out.type == FlatColumnType::VARCHAR) ensureScanPool(out.col_idx);
    } else {
      ensureBuildCol(out.step_idx, out.col_idx);
      if (out.type == FlatColumnType::VARCHAR)
        ensureBuildPool(out.step_idx, out.col_idx);
    }
  }

  // Declare runtime helpers
  auto getOrDeclare = [&](const char *name, FunctionType *fty) {
    return mod->getOrInsertFunction(name, fty);
  };

  // aqp_pk_builder_append_int(void*, u32, i32)
  FunctionCallee fn_append_int = getOrDeclare("aqp_pk_builder_append_int",
      FunctionType::get(voidTy, {i8p, i32, i32}, false));
  // aqp_pk_builder_append_str(void*, u32, i8*, u32)
  FunctionCallee fn_append_str = getOrDeclare("aqp_pk_builder_append_str",
      FunctionType::get(voidTy, {i8p, i32, i8p, i32}, false));
  // aqp_pk_builder_finish_row(void*)
  FunctionCallee fn_finish_row = getOrDeclare("aqp_pk_builder_finish_row",
      FunctionType::get(voidTy, {i8p}, false));
  // memcmp for VARCHAR comparisons
  FunctionCallee fn_memcmp = getOrDeclare("memcmp",
      FunctionType::get(i32, {i8p, i8p, i64}, false));
  // aqp_str_contains
  FunctionCallee fn_str_contains = getOrDeclare("aqp_str_contains",
      FunctionType::get(i32, {i8p, i32, i8p, i32}, false));
  // aqp_like_match
  FunctionCallee fn_like_match = getOrDeclare("aqp_like_match",
      FunctionType::get(i32, {i8p, i32, i8p, i32}, false));

  Constant *EMPTY_CONST = ConstantInt::get(i32, 0xFFFFFFFFu);
  Constant *FIB_HASH = ConstantInt::get(i32, 2654435761u);

  b.CreateBr(loop_bb);

  // === Loop Header ===
  b.SetInsertPoint(loop_bb);
  PHINode *row_phi = b.CreatePHI(i64, 2, "row");
  row_phi->addIncoming(start_arg, entry_bb);
  Value *loop_cond = b.CreateICmpUGE(row_phi, end_arg, "done");
  BasicBlock *body_bb = BasicBlock::Create(*ctx, "body", fn);
  b.CreateCondBr(loop_cond, exit_bb, body_bb);

  b.SetInsertPoint(body_bb);

  // === Helper: Load INT32 from scan column ===
  auto loadScanInt32 = [&](int col_idx, Value *row) -> Value * {
    Value *ptr = b.CreateBitCast(scan_col_ptrs[col_idx], i32p);
    return b.CreateLoad(i32, b.CreateGEP(i32, ptr, row),
                        "scan_i32_" + std::to_string(col_idx));
  };

  // === Helper: Load VARCHAR from scan column → (ptr, len) ===
  auto loadScanVarchar = [&](int col_idx, Value *row) -> std::pair<Value*, Value*> {
    Value *offsets_ptr = b.CreateBitCast(scan_col_ptrs[col_idx], i32p);
    Value *start_off = b.CreateLoad(i32, b.CreateGEP(i32, offsets_ptr, row), "vchar_start");
    Value *next_row = b.CreateAdd(row, ConstantInt::get(i64, 1));
    Value *end_off = b.CreateLoad(i32, b.CreateGEP(i32, offsets_ptr, next_row), "vchar_end");
    Value *len = b.CreateSub(end_off, start_off, "vchar_len");
    Value *pool = scan_pool_ptrs[col_idx];
    Value *str_ptr = b.CreateGEP(i8, pool, b.CreateZExt(start_off, i64), "vchar_ptr");
    return {str_ptr, len};
  };

  // === Helper: Emit one filter descriptor check ===
  // Returns the basic block to jump to on success. Uses bb_fail for failure.
  // Sets insertion point to the success block.
  std::function<void(const PipelineFilterDesc &, Value *, BasicBlock *)>
  emitFilter = [&](const PipelineFilterDesc &fd, Value *row, BasicBlock *fail_bb) {
    using K = PipelineFilterDesc::Kind;

    if (fd.kind == K::UNSUPPORTED) {
      // Can't JIT this filter — abort compilation will be caught at top level
      b.CreateBr(fail_bb);
      BasicBlock *unreachable_bb = BasicBlock::Create(*ctx, "unreachable", fn);
      b.SetInsertPoint(unreachable_bb);
      return;
    }

    // Null check for nullable columns
    auto emitNullCheck = [&](int col_idx, BasicBlock *null_fail_bb) {
      // For now, skip null check for non-nullable columns
      // Nullable columns check: bit in null_bitmap
    };
    (void)emitNullCheck;

    switch (fd.kind) {
    case K::INT32_EQ: case K::INT32_NE:
    case K::INT32_LT: case K::INT32_GT:
    case K::INT32_LE: case K::INT32_GE: {
      Value *val = loadScanInt32(fd.col_idx, row);
      Value *c = ConstantInt::get(i32, static_cast<uint32_t>(fd.int_const));
      Value *cmp;
      switch (fd.kind) {
      case K::INT32_EQ: cmp = b.CreateICmpEQ(val, c); break;
      case K::INT32_NE: cmp = b.CreateICmpNE(val, c); break;
      case K::INT32_LT: cmp = b.CreateICmpSLT(val, c); break;
      case K::INT32_GT: cmp = b.CreateICmpSGT(val, c); break;
      case K::INT32_LE: cmp = b.CreateICmpSLE(val, c); break;
      case K::INT32_GE: cmp = b.CreateICmpSGE(val, c); break;
      default: cmp = ConstantInt::getTrue(*ctx); break;
      }
      BasicBlock *pass_bb = BasicBlock::Create(*ctx, "filt_pass", fn);
      b.CreateCondBr(cmp, pass_bb, fail_bb);
      b.SetInsertPoint(pass_bb);
      break;
    }
    ...
    case K::INT32_IN_SET: {
      ...
      for (int32_t v : fd.int_set) {
        Value *cmp = b.CreateICmpEQ(val, ConstantInt::get(i32, static_cast<uint32_t>(v)));
        match = b.CreateOr(match, cmp);
      }
      if (fd.negated) match = b.CreateNot(match);
      BasicBlock *pass_bb = BasicBlock::Create(*ctx, "in_pass", fn);
      b.CreateCondBr(match, pass_bb, fail_bb);
      b.SetInsertPoint(pass_bb);
      break;
    }

    case K::VARCHAR_EQ: {
      auto [str_ptr, str_len] = loadScanVarchar(fd.col_idx, row);
      uint32_t const_len = static_cast<uint32_t>(fd.str_const.size());
      Value *len_match = b.CreateICmpEQ(str_len, ConstantInt::get(i32, const_len));
      BasicBlock *len_ok_bb = BasicBlock::Create(*ctx, "veq_len_ok", fn);
      BasicBlock *veq_result_bb = BasicBlock::Create(*ctx, "veq_result", fn);
      b.CreateCondBr(len_match, len_ok_bb, fd.negated ? veq_result_bb : fail_bb);

      b.SetInsertPoint(len_ok_bb);
      // Embed constant string as global
      Constant *const_str = ConstantDataArray::getString(*ctx, fd.str_const, false);
      auto *gv = new GlobalVariable(*mod, const_str->getType(), true,
                                     GlobalValue::PrivateLinkage, const_str);
      Value *const_ptr = b.CreateBitCast(gv, i8p);
      Value *cmp_result = b.CreateCall(fn_memcmp,
          {str_ptr, const_ptr, ConstantInt::get(i64, const_len)});
      Value *eq = b.CreateICmpEQ(cmp_result, ConstantInt::get(i32, 0));
      if (fd.negated) eq = b.CreateNot(eq);
      BasicBlock *pass_bb = BasicBlock::Create(*ctx, "veq_pass", fn);
      b.CreateCondBr(eq, pass_bb, fail_bb);

      if (fd.negated) {
        // Handle the len_mismatch → pass path for negated
        b.SetInsertPoint(veq_result_bb);
        b.CreateBr(pass_bb);
      }
    ...
    case K::VARCHAR_LIKE_PREFIX: {
      auto [str_ptr, str_len] = loadScanVarchar(fd.col_idx, row);
      uint32_t pfx_len = static_cast<uint32_t>(fd.str_const.size());
      Value *len_ok = b.CreateICmpUGE(str_len, ConstantInt::get(i32, pfx_len));
      BasicBlock *len_ok_bb = BasicBlock::Create(*ctx, "vpfx_len_ok", fn);
      b.CreateCondBr(len_ok, len_ok_bb, fail_bb);
      b.SetInsertPoint(len_ok_bb);

      Constant *pfx_str = ConstantDataArray::getString(*ctx, fd.str_const, false);
      auto *gv = new GlobalVariable(*mod, pfx_str->getType(), true,
                                     GlobalValue::PrivateLinkage, pfx_str);
      Value *pfx_ptr = b.CreateBitCast(gv, i8p);
      Value *cmp = b.CreateCall(fn_memcmp,
          {str_ptr, pfx_ptr, ConstantInt::get(i64, pfx_len)});
      Value *match = b.CreateICmpEQ(cmp, ConstantInt::get(i32, 0));
      if (fd.negated) match = b.CreateNot(match);
      BasicBlock *pass_bb = BasicBlock::Create(*ctx, "vpfx_pass", fn);
      b.CreateCondBr(match, pass_bb, fail_bb);
      b.SetInsertPoint(pass_bb);
      break;
    }

    case K::VARCHAR_LIKE_SUFFIX: {
      auto [str_ptr, str_len] = loadScanVarchar(fd.col_idx, row);
      uint32_t sfx_len = static_cast<uint32_t>(fd.str_const.size());
      Value *len_ok = b.CreateICmpUGE(str_len, ConstantInt::get(i32, sfx_len));
      BasicBlock *len_ok_bb = BasicBlock::Create(*ctx, "vsfx_len_ok", fn);
      b.CreateCondBr(len_ok, len_ok_bb, fail_bb);
      b.SetInsertPoint(len_ok_bb);

      Constant *sfx_str = ConstantDataArray::getString(*ctx, fd.str_const, false);
      auto *gv = new GlobalVariable(*mod, sfx_str->getType(), true,
                                     GlobalValue::PrivateLinkage, sfx_str);
      Value *sfx_ptr = b.CreateBitCast(gv, i8p);
      Value *offset = b.CreateSub(str_len, ConstantInt::get(i32, sfx_len));
      Value *end_ptr = b.CreateGEP(i8, str_ptr, b.CreateZExt(offset, i64));
      Value *cmp = b.CreateCall(fn_memcmp,
          {end_ptr, sfx_ptr, ConstantInt::get(i64, sfx_len)});
      Value *match = b.CreateICmpEQ(cmp, ConstantInt::get(i32, 0));
      if (fd.negated) match = b.CreateNot(match);
      BasicBlock *pass_bb = BasicBlock::Create(*ctx, "vsfx_pass", fn);
      b.CreateCondBr(match, pass_bb, fail_bb);
      b.SetInsertPoint(pass_bb);
      break;
    }

    case K::VARCHAR_LIKE_CONTAINS: {
      auto [str_ptr, str_len] = loadScanVarchar(fd.col_idx, row);
      Constant *needle_str = ConstantDataArray::getString(*ctx, fd.str_const, false);
      auto *gv = new GlobalVariable(*mod, needle_str->getType(), true,
                                     GlobalValue::PrivateLinkage, needle_str);
      Value *needle_ptr = b.CreateBitCast(gv, i8p);
      Value *result = b.CreateCall(fn_str_contains,
          {str_ptr, str_len, needle_ptr, ConstantInt::get(i32, (uint32_t)fd.str_const.size())});
      Value *match = b.CreateICmpNE(result, ConstantInt::get(i32, 0));
      if (fd.negated) match = b.CreateNot(match);
      BasicBlock *pass_bb = BasicBlock::Create(*ctx, "vcont_pass", fn);
      b.CreateCondBr(match, pass_bb, fail_bb);
      b.SetInsertPoint(pass_bb);
      break;
    }

    case K::VARCHAR_LIKE_EXACT:
    case K::VARCHAR_LIKE_SEGMENTS:
    case K::VARCHAR_LIKE_FULL: {
      // Use aqp_like_match for all complex patterns
      auto [str_ptr, str_len] = loadScanVarchar(fd.col_idx, row);
      const std::string &pat = fd.str_const;
      Constant *pat_str = ConstantDataArray::getString(*ctx, pat, false);
      auto *gv = new GlobalVariable(*mod, pat_str->getType(), true,
                                     GlobalValue::PrivateLinkage, pat_str);
      Value *pat_ptr = b.CreateBitCast(gv, i8p);
      Value *result = b.CreateCall(fn_like_match,
          {str_ptr, str_len, pat_ptr, ConstantInt::get(i32, (uint32_t)pat.size())});
      Value *match = b.CreateICmpNE(result, ConstantInt::get(i32, 0));
      if (fd.negated) match = b.CreateNot(match);
      BasicBlock *pass_bb = BasicBlock::Create(*ctx, "vlike_pass", fn);
      b.CreateCondBr(match, pass_bb, fail_bb);
      b.SetInsertPoint(pass_bb);
      break;
    }
    ...
    case K::LOGICAL_AND: {
      if (fd.children.size() == 2) {
        emitFilter(fd.children[0], row, fail_bb);
        emitFilter(fd.children[1], row, fail_bb);
      }
      break;
    }
    ...
    case K::LOGICAL_OR: {
      if (fd.children.size() == 2) {
        BasicBlock *try_right = BasicBlock::Create(*ctx, "or_right", fn);
        BasicBlock *or_pass = BasicBlock::Create(*ctx, "or_pass", fn);
        // Try left — if it fails, try right
        BasicBlock *saved_insert = b.GetInsertBlock();
        (void)saved_insert;
        // We need a different approach: emit left with fail→try_right
        emitFilter(fd.children[0], row, try_right);
        b.CreateBr(or_pass);
        b.SetInsertPoint(try_right);
        emitFilter(fd.children[1], row, fail_bb);
        b.CreateBr(or_pass);
        b.SetInsertPoint(or_pass);
      }
      break;
    }

    case K::LOGICAL_NOT: {
      if (fd.children.size() == 1) {
        // Invert: if child passes, we fail; if child fails, we pass
        BasicBlock *child_pass = BasicBlock::Create(*ctx, "not_child_pass", fn);
        BasicBlock *not_pass = BasicBlock::Create(*ctx, "not_pass", fn);
        emitFilter(fd.children[0], row, not_pass);
        b.CreateBr(child_pass);
        b.SetInsertPoint(child_pass);
        b.CreateBr(fail_bb);
        b.SetInsertPoint(not_pass);
      }
      break;
    }

    default:
      // UNSUPPORTED, IS_NULL, IS_NOT_NULL, VARCHAR_IN_SET
      // Fall through — these are rare in JOB and can be added later
      b.CreateBr(fail_bb);
      BasicBlock *unr = BasicBlock::Create(*ctx, "unsup", fn);
      b.SetInsertPoint(unr);
      break;
    }
  };

  // Check if all filter descriptors are JIT-able (recursive)
  std::function<bool(const PipelineFilterDesc &)> isJittable =
      [&](const PipelineFilterDesc &fd) -> bool {
    if (fd.kind == PipelineFilterDesc::UNSUPPORTED) return false;
    if (fd.kind == PipelineFilterDesc::IS_NULL) return false;
    if (fd.kind == PipelineFilterDesc::IS_NOT_NULL) return false;
    if (fd.kind == PipelineFilterDesc::VARCHAR_IN_SET) return false;
    for (const auto &ch : fd.children) {
      if (!isJittable(ch)) return false;
    }
    return true;
  };
  bool all_filters_jittable = true;
  for (const auto &fd : plan.scan_filter_descs) {
    if (!isJittable(fd)) {
      all_filters_jittable = false;
      break;
    }
  }
  if (!all_filters_jittable) return nullptr;

  // === Emit scan filters (short-circuit AND chain) ===
  for (const auto &fd : plan.scan_filter_descs) {
    emitFilter(fd, row_phi, loop_footer);
  }

  // === Emit hash probe for each join step ===
  // For Phase 1, handle semi-join-only (all steps is_semi).
  // For inner join, we need the iterative DFS — added in Phase 2.

  bool all_semi = true;
  for (const auto &step : plan.join_steps) {
    if (!step.is_semi) { all_semi = false; break; }
  }

  // Phase 1: Semi-join only path
  if (!all_semi) return nullptr; // Phase 2 will handle inner joins

  for (size_t si = 0; si < nsteps; si++) {
    const auto &step = plan.join_steps[si];
    if (step.probe_step_idx >= 0) return nullptr;
    Value *probe_key = loadScanInt32(step.scan_key_col, row_phi);

    // Fibonacci hash
    Value *hash = b.CreateAnd(
        b.CreateMul(probe_key, FIB_HASH),
        step_ssa[si].mask, "hash_" + std::to_string(si));
    Value *hash_ext = b.CreateZExt(hash, i64);

    // Load initial bucket
    Value *chain_idx = b.CreateLoad(
        i32, b.CreateGEP(i32, step_ssa[si].buckets, hash_ext),
        "chain_" + std::to_string(si));

    // Chain walk loop
    BasicBlock *chain_loop_bb = BasicBlock::Create(*ctx, "chain_" + std::to_string(si), fn);
    BasicBlock *chain_check_bb = BasicBlock::Create(*ctx, "chain_check_" + std::to_string(si), fn);
    BasicBlock *chain_advance_bb = BasicBlock::Create(*ctx, "chain_adv_" + std::to_string(si), fn);
    BasicBlock *step_pass_bb = BasicBlock::Create(*ctx, "step_pass_" + std::to_string(si), fn);

    BasicBlock *chain_entry_bb = b.GetInsertBlock();
    b.CreateBr(chain_loop_bb);
    b.SetInsertPoint(chain_loop_bb);
    PHINode *idx_phi = b.CreatePHI(i32, 2, "idx_" + std::to_string(si));
    idx_phi->addIncoming(chain_idx, chain_entry_bb);

    // Check EMPTY
    Value *is_empty = b.CreateICmpEQ(idx_phi, EMPTY_CONST);
    b.CreateCondBr(is_empty, loop_footer, chain_check_bb);

    b.SetInsertPoint(chain_check_bb);
    Value *idx_ext = b.CreateZExt(idx_phi, i64);
    Value *ht_key = b.CreateLoad(i32, b.CreateGEP(i32, step_ssa[si].keys, idx_ext));
    Value *key_match = b.CreateICmpEQ(ht_key, probe_key);
    b.CreateCondBr(key_match, step_pass_bb, chain_advance_bb);

    b.SetInsertPoint(chain_advance_bb);
    Value *next_idx = b.CreateLoad(i32, b.CreateGEP(i32, step_ssa[si].next, idx_ext));
    idx_phi->addIncoming(next_idx, chain_advance_bb);
    b.CreateBr(chain_loop_bb);

    b.SetInsertPoint(step_pass_bb);
  }

  // === Emit output row ===
  BasicBlock *emit_bb = b.GetInsertBlock(); // current insert point after all step passes
  (void)emit_bb;

  // Phase 1: Only INT32 output columns. VARCHAR output falls back to interpreted.
  for (const auto &out : plan.output_cols) {
    if (out.type != FlatColumnType::INT32) return nullptr;
  }

  for (size_t c = 0; c < plan.output_cols.size(); c++) {
    const auto &out = plan.output_cols[c];
    if (out.source == KernelOutputCol::FROM_SCAN) {
      Value *val = loadScanInt32(out.col_idx, row_phi);
      b.CreateCall(fn_append_int,
          {builder_arg, ConstantInt::get(i32, c), val});
    } else {
      // FROM_JOIN: load from build table
      int si = out.step_idx;
      // For semi-join, we don't have matched_rows — this shouldn't happen
      // (semi-join means no output from build side)
      // But if it does, we'd need the matched row ID
      // For Phase 1 (semi-only), all output should be FROM_SCAN
      // If we see FROM_JOIN with semi, it's a bug in plan analysis
      // Safety: emit via runtime call
      return nullptr;
    }
  }

  b.CreateCall(fn_finish_row, {builder_arg});
  b.CreateBr(loop_footer);

  // === Loop Footer ===
  b.SetInsertPoint(loop_footer);
  Value *row_next = b.CreateAdd(row_phi, ConstantInt::get(i64, 1));
  row_phi->addIncoming(row_next, loop_footer);
  b.CreateBr(loop_bb);
  ...
  if (verifyFunction(*fn, &es)) {
#ifndef NDEBUG
    std::cerr << "[AQP-JIT] verifyFunction failed (pipeline kernel): " << es.str() << "\n";
    fn->print(errs());
#endif
    return nullptr;
  }
...
#ifndef NDEBUG
  std::cerr << "[AQP-JIT] compiled pipeline kernel fn=" << fn_name
            << "  scan_filters=" << plan.scan_filter_descs.size()
            << "  join_steps=" << nsteps
            << "  out_cols=" << plan.output_cols.size() << "\n";
#endif

  return jitTargetAddressToFunction<AQPPipelineKernelFn>(sym->getAddress());
}
